...
- `pending` かつ `deferred_reason` あり(**持ち越し**): レートリミットで中断されたタスク。**失敗ではない**ことを明確に伝える。`rate_limit_reset_at`(枠の解除予定)と `step`(どこまで進んでいたか)、退避先の`diagnostic_branch`を提示し、「次に`night-run/run.sh start`すればそのまま続きから再開される(`--retry-failed`は不要)」と案内する
- `pending`: まだ着手されていないことを伝える

あわせてトップレベルの `run_summary`(直近の実行の結果)があれば読む。**`stopped_reason` が「実行中」で始まっている、または `finished_at` が無い場合は、その実行は正常終了していない**(クラッシュしたか `run.sh stop` で止められたか、今まさに実行中)。Step 4のコンテナ確認と突き合わせて、「実行中」なのか「途中で落ちた」のかを判断して伝える——前回の実行結果を今回の結果として報告しないこと。`stopped_reason` が、その回がなぜ終わったか(全タスク完了 / 締切到達 / **タスク数上限** / **予算上限** / **レートリミット**)を持っている。上限で止まっている場合は「異常ではなく、契約プランの枠を使い切らないための設計上の打ち切り」であることを伝え、残りを進めたいなら再度`start`するか`limits`の`max_tasks_per_run`を上げる選択肢があると案内する(上げれば翌日の対話利用の枠を削る点も添える)。`tasks_started`(着手件数)・`spent_usd`(その回の消費額。サブスクリプション認証では記録されないことがある)・`limits_description`(適用された設定)も報告に含める。
...
- 完了タスク数・失敗タスク数・**持ち越しタスク数**・未着手タスク数
- その回がなぜ終わったか(`run_summary.stopped_reason`)と消費量
- 失敗タスクがあれば、原因と診断ブランチ
- 次にユーザーがとるべきアクション(あれば): 例えば「`failed`タスクの診断ブランチを確認する」「持ち越しがあるので今夜もう一度`night-run/run.sh start`する」「コンテナが落ちているので`night-run/run.sh start`でやり直す前に原因を確認する」等
