...
- **v7(2026-09-16)で変更 — サブスクリプション契約前提の持ち越し**:上記のbackoff(最大5回・合計1時間強)はAPIキー課金のレートリミット(数分〜数十分で解ける)を想定した値で、**サブスクリプション契約(Pro/Max)の5時間ローリング枠には届かない**。実際には「1時間待ってからfailed」がほぼ確定で、待ち時間もそこまでのトークンも無駄になっていた。そこで検知時に次のように判断する(`plan_rate_limit_wait`):
  - エラー本文から解除時刻を読む(`Claude AI usage limit reached|<epoch>` 形式のepoch秒、またはISO8601)
  - 解除時刻が分かり、待った後も締切まで実作業の時間(`MIN_USEFUL_REMAINING_SECONDS`=15分)が残る → その時刻+`RATE_LIMIT_RESET_MARGIN_SECONDS`(2分)まで待って再開
  - 締切に間に合わない、または解除時刻が読めずbackoffの回数上限に達した → **待たずに持ち越す**(`mark_task_deferred`)
  - 持ち越しは`failed`にせず`status`を`pending`のまま保ち、`deferred_reason`/`deferred_at`/`rate_limit_reset_at`を記録する。`failed`にすると(1)翌回は`run.sh start --retry-failed`を人が叩かない限り拾われない (2)実装が壊れた失敗と区別がつかない、の2点で運用上困るため
  - 持ち越しが起きた時点でその回の実行自体を終える(枠が空いていないのに次のタスクへ進んでも同じところで止まるだけ)。`run_task_with_retry`が`"deferred"`を返し、`main()`がループを抜ける

### 4.4 消費量の上限(v7で追加)

サブスクリプション契約では、夜間実行の消費が翌日の対話利用の枠を直接削る。**1タスクあたりの`--max-budget-usd`だけでは歯止めにならない**(サブスクリプション認証ではコストが報告されないことがあり、その場合この上限は機能しない)ため、件数ベースの上限を主の歯止めとして併用する。
...
| `autocompact` | `150k` | `--autocompact`に渡す値。文脈がこの大きさを超えたら要約へ置き換える(`auto`、または100k〜1M。空で指定しない)。1タスクは最大60分走るので、放置すると文脈が伸び続け、その全部を毎リクエスト読み直すことになる |
...
- **v7で追加**:トップレベルに`limits`(消費量の設定、4.4節)と`run_summary`(直近の実行の着手件数・消費額・終了理由)を持つ。どちらも省略可能で、`limits`が無ければ既定値が使われる。タスクエントリ側には`total_cost_usd`/`usage`(issue #47)と、持ち越し時の`deferred_reason`/`deferred_at`/`rate_limit_reset_at`が加わる
