...
> **結論を先に**: 推奨は **案B(KGP先行)**。
> #82 / #83 / #84 / #86 / #88 を取り込む(**#88 は #105 より先**。#85 と #89 はCIが緑でも
> 別途判断が要る)→ KGP警告の残りを潰す →
...
3. **その後で #105(`deviceId` 削除)をマージ。** 順序を逆にすると #88 が自動クローズされ、
   device_info_plus のKGP解消がやりにくくなる**おそれがある**(1-3節)。
   自動クローズされるかどうかは未検証の仮説(H9)だが、**#88 を先に入れておけば
   当たり外れに関係なく済む**ため、リスク回避としてこの順序を採る
