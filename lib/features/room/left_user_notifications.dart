...
/// 誰かが離脱(users/{uid}がRTDBから消えた)したら、待機画面・ゲーム画面の
/// どちらでも「(名前)さんが抜けました」とSnackBarで知らせるフック。
///
/// 直前のusersスナップショットをuseRefで保持し、次のスナップショットとの
/// 差分(detectLeftUsers)から離脱者を検出する。このウィジェットが破棄され
/// たら追跡もやめてよい一時状態なのでhooksで持つ(AGENTS.mdの状態管理規約)。
