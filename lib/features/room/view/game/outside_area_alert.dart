...
/// 「戻るまで◯◯が続きます」の補足文。
///
/// 以前は「アプリを開いている間」と限定していた。判定がGamePageの毎秒の
/// 再描画で回っており、画面を消すとフレームごと止まって振動も通知も
/// 止まっていたため、モック2a-07の「戻るまで振動と通知が続きます」を
/// そのまま出すと果たせない約束になっていた。
///
/// issue #71 で判定を描画から切り離し(`GameAlerts`の自前タイマー)、画面を
/// 消していても検知・解除されるようになったので、限定を外してモックの
/// 文言に戻している。
const outsideAreaBannerSubtitle = 'エリアに戻るまで振動と通知が続きます';
